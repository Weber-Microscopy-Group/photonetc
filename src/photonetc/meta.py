...
def _set_attr(obj, name: str, value: Any):
    ...
    """
    typ = obj.__annotations__["attrs"]
    base = typ.__annotations__[name]
    if get_origin(base) is NotRequired:
        (inner,) = get_args(base)
        base = inner
    ...
    try:
        ...
        raise TypeError(f"field {name} annotation is invalid, expected two arguments")
    ...
    attrs: dict = obj.attrs  # type: ignore
    if isinstance(value, np.ndarray):
        ...
        if len(value) != shape[0]:
            raise ValueError(f"incorrect shape, expected {shape} found {(len(value),)}")
