...
def test_attribute_list():
    @pe.datacube.attribute_list
    class TestAttrs:
        nodef: str = pe.datacube.attribute("nodef")
        defval_str: str = pe.datacube.attribute("defval_str", default="defval_str")
        deffac_str: str = pe.datacube.attribute(
            "deffac_str", default_factory=lambda: "deffac_str"
        )

    attrs = TestAttrs(nodef="filled")
    assert attrs.nodef == "filled"
    assert attrs.defval_str == "defval_str"
    assert attrs.deffac_str == "deffac_str"

    attrs.nodef = "new"
    assert attrs.nodef == "new"

    # Typically, this should not be accessed by users.
    # Thus, it is correct behavior that type checkers show
    # an attribute error.
    assert attrs._nodef.name == "nodef"
    assert attrs._defval_str.name == "defval_str"
    assert attrs._deffac_str.name == "deffac_str"
